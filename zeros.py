...
# --- 2. KULLANICI ARAYÜZÜ (DÜZELTİLMİŞ) ---

def select_instruments_ui():
    root = tk.Tk()
    root.title("Enstrüman Seçimi")
    ...
    # --- PENCEREYİ ZORLA ÖNE ALMA KOMBİNASYONU ---
    root.lift()
    ...
    lbl = tk.Label(root, text="Hesaplamaya Dahil Edilecekler", font=("Arial", 11, "bold"))
    lbl.pack(pady=20)
    ...
    # BUTON RENGİNİ KALDIRDIM (Sistem Varsayılanı Kullanılsın)
    # Bu sayede yazı her zaman görünür olacaktır.
    tk.Button(root, text="HESAPLAMAYI BAŞLAT", command=on_submit, height=2, width=20).pack(pady=25)
...
def calculate_final_stable():
    ...
    output_file = 'sonuc_zero_curve_full.xlsx'
    curve_data = [] 

    print("--- FULL ZERO CURVE ANALİZİ ---")
    ...
    try:
        ...
        try:
            ...
            messagebox.showerror("Hata", "Dosya açık! Lütfen Excel'i kapat.")
            return
        ...
        # Piyasa Verisi
        fig.add_trace(go.Scatter(x=df_curve['Gün'], y=df_curve['Zero Spot Rate (%)'], mode='markers', name='Piyasa (Bootstrap)', marker=dict(size=10, color='black')))
        
        # Modeller
        fig.add_trace(go.Scatter(x=x_smooth, y=[nelson_siegel_svensson(nss_params, t) for t in x_smooth], mode='lines', name='NSS', line=dict(color='red', width=3)))
        ...
        fig.add_trace(go.Scatter(x=x_smooth, y=f_lin_reg(x_smooth), mode='lines', name='LinReg', line=dict(color='blue', width=2, dash='dot')))
